...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-[#faf8f4] overflow-hidden">
      ...
      {/* Mobile header — white top bar, only visible on small screens */}
      <header className="lg:hidden fixed top-0 left-0 right-0 z-30 h-14 bg-white border-b border-[#e2ddd5] flex items-center px-4 gap-3 flex-shrink-0">
        ...
        <Link href="/" className="flex items-center gap-2 min-w-0">
          <div
            className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold font-playfair flex-shrink-0"
            style={{ backgroundColor: GOLD, color: SIDEBAR_BG }}
          >
            ANW
          </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          <div
            className="w-72 max-w-[85vw] h-full flex flex-col shadow-2xl relative"
            style={{ backgroundColor: SIDEBAR_BG }}
          >
            <button
              onClick={() => setMobileOpen(false)}
              className="absolute top-3.5 right-3 p-1.5 rounded-lg transition-colors z-10 hover:bg-[#F3ECDA]/[0.08]"
              style={{ color: CREAM }}
